...
def add_to_bag(request, item_id):
    """
    Add a qty of the specified product or course to the shopping bag
    """
    try:
        ...
        if item_type == "course":
            ...
            if quantity + existing_in_bag > spaces_left:
                ...
                messages.error(
                    request,
                    f"Only {allowed} spot(s) left for {item.title}."
                )
        ...
        # Product
            ...
            if quantity + existing_in_bag > item.stock_quantity:
                ...
                messages.error(
                    request,
                    f"Only {allowed} of {item.name} left in stock."
                )
                return redirect(redirect_url)
        ...
        if item_key in bag:
            ...
            messages.success(
                request,
                f'Updated with x{quantity} {item_name}'
            )
        else:
            bag[item_key] = quantity
            messages.success(
                request,
                f'Added {item_name} to your bag'
            )
        ...
        messages.error(
            request,
            "Something went wrong while adding to your bag"
        )
        return redirect(request.POST.get('redirect_url', 'view_bag'))
...
def adjust_bag(request, item_id):
    ...
    try:
        ...
        if item_type == "course":
            ...
            if quantity > 0:
                if quantity > spaces_left:
                    messages.error(
                        request,
                        f"Only {spaces_left} spot(s) available "
                        f"for {item_name}."
                    )
                    return redirect(redirect_url)
                bag[item_key] = quantity
                ...
                if item_key in bag:
                    ...
                    messages.success(
                        request,
                        f'Removed {item_name} from your bag'
                    )
        ...
        # Product
            ...
            if quantity > 0:
                if quantity > item.stock_quantity:
                    messages.error(
                        request,
                        f"Only {item.stock_quantity} of {item_name} "
                        f"left in stock."
                    )
                    return redirect(redirect_url)
                bag[item_key] = quantity
        ...
        messages.error(
            request,
            "Something went wrong while updating your bag"
        )
        return redirect(request.POST.get('redirect_url', 'view_bag'))
